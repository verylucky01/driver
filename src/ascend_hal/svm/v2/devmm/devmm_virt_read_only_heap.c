...
DVresult devmm_init_read_only_heap_by_devid(uint32_t device, uint32_t heap_sub_type)
{
    ...
    devmm_fill_read_only_heap_type(device, heap_sub_type, &heap_type);
    devmm_fill_read_only_heap_info(mgmt, &heap_type, device, &heap_info);
    ret = devmm_virt_init_heap_customize(mgmt, &heap_type, &heap_info, &g_read_only_heap_op);
    if (ret != DRV_ERROR_NONE) {
        DEVMM_DRV_WARN("Init read heap fail. (ret=%d; devid=%u)\n", ret, device);
    }
    return ret;
}
