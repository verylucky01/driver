...
void dcmi_log_record(const char *log_file, const char *log_file_bak, const char *log_buf);

int dcmi_board_type_is_sei()
{
...
int dcmi_get_log_dir(char *log_dir, int log_dir_size)
{
    int ret;
...
#ifndef _WIN32
static int dcmi_get_user_name_linux(char *user_name, int name_len)
{
    int ret;
    struct passwd *user_info = NULL;
...
int dcmi_get_ip_linux(char *user_ip, unsigned int ip_len)
{
    int ip_length = 0;
...
static int dcmi_get_user_name_and_ip_linux(char *user_name, int name_len, char *user_ip, unsigned int ip_len)
{
    int ret;

    /* 获取userName */
    ret = dcmi_get_user_name_linux(user_name, name_len);
    if (ret != 0) {
        return ret;
    }

    /* 获取userIP */
    ret = dcmi_get_ip_linux(user_ip, ip_len);
    return ret;
}
...
static int dcmi_get_user_name_and_ip_win(char *user_name, int name_len, char *user_ip, int ip_len)
{
...
static int dcmi_get_username_and_ip(char *user_name, int name_len, char *user_ip, unsigned int ip_len)
{
#ifndef _WIN32
    return dcmi_get_user_name_and_ip_linux(user_name, name_len, user_ip, ip_len);
#else
    return dcmi_get_user_name_and_ip_win(user_name, name_len, user_ip, ip_len);
#endif
}

static void dcmi_get_curr_time_str(char *curr_time_str, int curr_time_str_size)
{
...
STATIC void dcmi_log_backup(const char *log_file, const char *log_file_bak)
{
...
STATIC FILE *dcmi_get_log_file_handle(const char *log_file)
{
    FILE *fp = NULL;
    int ret;
...
#endif
    ret = dcmi_file_realpath_allow_nonexist(log_file, path, sizeof(path));
    if (ret != DCMI_OK) {
...
static int dcmi_get_log_file(const char *log_dir, const char *mod, char *log_file, int log_file_len)
{
...
static int dcmi_get_log_file_bak(const char *log_dir, const char *mod, char *log_file_bak, int log_file_bak_len)
{
    int ret;

#ifndef _WIN32
    const char *log_prefix = (dcmi_board_type_is_sei()) ? "ies_" : COMMON_LOG_PREFIX;
#else
...
int dcmi_chmod_log_file(const char *log_dir)
{
    struct passwd *user_info = NULL;
    user_info = getpwuid(getuid());
    if (user_info == NULL) {
...
void dcmi_log_fun(const char *mod, const char *fmt, ...)
{
    ...
    ret = dcmi_get_log_dir(log_dir, sizeof(log_dir));
    if (ret != 0) {
        return;
    }
    ...
    if (dcmi_chmod_log_file((const char *)log_dir) != 0) {
        return;
    }

    if (dcmi_get_log_file((const char *)log_dir, mod, log_file, sizeof(log_file)) != 0) {
        return;
    }
    if (dcmi_get_log_file_bak((const char *)log_dir, mod, log_file_bak, sizeof(log_file_bak)) != 0) {
        return;
    }
    ...
    dcmi_log_record(log_file, log_file_bak, (const char *)log_buf);
}
...
void dcmi_log_record(const char *log_file, const char *log_file_bak, const char *log_buf)
{
    ...
    fp = dcmi_get_log_file_handle(log_file);
    if (fp == NULL) {
        return;
    }
    ...
    if ((ftell(fp) + (long)log_len) > DCMI_LOGFILE_MAX_LEN) {
        (void)fclose(fp);
        dcmi_log_backup(log_file, log_file_bak);
#ifndef _WIN32
        char path[PATH_MAX + 1] = {0x00};
#else
        char path[MAX_PATH + 1] = {0x00};
#endif
        int ret = dcmi_file_realpath_allow_nonexist(log_file, path, sizeof(path));
        if (ret != DCMI_OK) {
            gplog(LOG_ERR, "file path is illegal errno(%d).\n", errno);
            return;
        }
    ...
    dcmi_get_curr_time_str(timestamp_str, sizeof(timestamp_str));

#ifndef DT_FLAG
    tid = GETTID();
    (void)dcmi_get_username_and_ip(user_name, USER_NAME_MAX_LEN, user_ip, USER_IP_MAX_LEN);
