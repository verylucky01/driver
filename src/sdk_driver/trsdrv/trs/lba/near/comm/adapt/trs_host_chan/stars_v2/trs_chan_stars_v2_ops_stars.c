...
{
    return ka_mm_readl(cqint->base + TRS_STARS_V2_HOST_CQINT_MID_STATUS_OFFSET(cqint->group));
}

static inline u32 _trs_chan_ops_stars_cqint_get_l2_status(struct trs_stars_cqint *cqint, u32 mid_bit)
{
    return ka_mm_readl(cqint->base + TRS_STARS_V2_HOST_CQINT_L2_STATUS_OFFSET(cqint->group, mid_bit));
}

static void _trs_chan_ops_stars_cqint_set_l2_ctrl(struct trs_stars_cqint *cqint, u32 mid_bit, u32 l2_status)
{
    ka_mm_writel(l2_status, cqint->base + TRS_STARS_V2_HOST_CQINT_L2_CTRL_OFFSET(cqint->group, mid_bit));
}
...
#ifndef EMU_ST
    if ((val & 0x1) == 0) {
        ka_mm_writel(1, cqint->base + TRS_STARS_V2_HOST_CQINT_L1_CTRL_OFFSET(cqint->group));
    }

    ka_mm_writel(((u32)val & 0x1U), cqint->base + TRS_STARS_V2_HOST_CQINT_L1_MASK_OFFSET(cqint->group));
#endif
}
