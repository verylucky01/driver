...
typedef union {
    ...
    struct {
         ...
         * stop SQCQ state machine into launching new blocks(only stop DVPP task at block boundary),
         * and when all launched block has been executed,an task_paused interrupt would be signaled
...
typedef union {
    ...
    struct {
         ...
         */
        unsigned int p0_mapped_in_active_sqid : 5;       /* [4:0] */
...
typedef union {
    ...
    struct {
         ...
         * while the DFX state is only available in the active SQ set,
