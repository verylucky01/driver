...
#include "virtmnghost_resource_sriov.h"

struct vmng_msg_chan_res g_vmngh_type_res_sriov[VMNG_MSG_CHAN_TYPE_MAX] = {
    /* admin */
    ...
    {VMNG_MSG_VPC_BASE_TX_PCIE, VMNG_MSG_VPC_NUM_TX_PCIE,
     VMNG_MSG_VPC_BASE_RX_PCIE, VMNG_MSG_VPC_NUM_RX_PCIE},
    {VMNG_MSG_VPC_BASE_TX_DVPP, VMNG_MSG_VPC_NUM_TX_DVPP,
     VMNG_MSG_VPC_BASE_RX_DVPP, VMNG_MSG_VPC_NUM_RX_DVPP},
...
struct vmng_msg_chan_res *vmngh_get_msg_cluster_res_sriov(enum vmng_msg_chan_type type)
{
    ...
    return &g_vmngh_type_res_sriov[type];
}
    ...
    if (vmng_is_blk_chan(chan_type) == true) {
        for (i = 0; i < res->tx_num; i++) {
            irq_array->tx_finish_irq[i] =
                VMNG_DB_BASE_MSG_BLOCK_TX_FINISH + (res->tx_base - VMNG_MSG_BASE_TX_BLOCK) + i;
        }
        for (i = 0; i < res->rx_num; i++) { /* alloc for block, beyond msg irq */
            irq_array->rx_resp_irq[i] = VMNG_MSIX_BASE_MSG_BLOCK_RX_REPLY + (res->rx_base - VMNG_MSG_BASE_RX_BLOCK) + i;
        }
    }
}
