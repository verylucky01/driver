...
static dev_t g_davinci_intf_extra_dev;
...
STATIC atomic64_t g_recycle_wq_idx;
...
STATIC int check_proc_open_module_count(
    pid_t owner_pid, TASK_TIME_TYPE start_time, const char *module_name, unsigned int max)
{
...
STATIC signed int drv_ascend_intf_extra_setup_cdev(const struct file_operations *fops)
{
    ...
    ret = alloc_chrdev_region(&g_davinci_intf_extra_dev, 0, 1, "hisi_davinci_intf");
    if (ret != 0) {
        log_intf_err("Failed to invoke alloc_chrdev_region for davinci_manager_extra. (ret=%d)\n", ret);
        return -EINVAL;
    }

    g_cdev_extra.owner = THIS_MODULE;
    cdev_init(&g_cdev_extra, fops);
    ret = cdev_add(&g_cdev_extra, g_davinci_intf_extra_dev, 1);
    if (ret < 0) {
        log_intf_err("Failed to invoke cdev_add for davinci_manager_extra. (ret=%d)\n", ret);
        goto unregister_region;
    }

    dev = device_create(g_davinci_class, NULL, g_davinci_intf_extra_dev, NULL, DAVINCI_MANAGER_EXTRA_NAME);
    if (IS_ERR(dev)) {
        ret = PTR_ERR(dev);
        log_intf_err("Failed to invoke device_create for davinci_manager_extra. (ret=%d)\n", ret);
        goto delete_cdev;
    }
...
delete_cdev:
    cdev_del(&g_cdev_extra);
unregister_region:
    unregister_chrdev_region(g_davinci_intf_extra_dev, 1);
...
{
    cdev_del(&g_cdev_extra);
    device_destroy(g_davinci_class, g_davinci_intf_extra_dev);
    unregister_chrdev_region(g_davinci_intf_extra_dev, 1);
    return;
}
#endif
...
STATIC void drv_davinci_intf_check_process(void)
{
    ...
    if (list_empty_careful(process_list) != 0) {
        return;
    }

    list_for_each_safe(pos, next, process_list)
    {
        proc_node = (struct davinci_intf_process_stru *)list_entry(pos, struct davinci_intf_process_stru, list);
        if (list_empty_careful(&proc_node->file_list) && (atomic_read(&proc_node->work_count) <= 0)) {
            list_del(pos);
...
STATIC void drv_davinci_run_release(struct work_struct *work)
{
    ...
    free_list = container_of(work, struct davinci_intf_free_list_stru, release_work);
    if (free_list != NULL) {
    ...
    cpumask_var_t cpu_mask)
{
    ...
    if (cpumask_available(cpu_mask)) {
        /* set cpumask */
        (void)set_cpus_allowed_ptr(release_task, cpu_mask);
    }
#endif
    return;
}
...
void drv_ascend_release_work(struct davinci_intf_free_list_stru *free_list)
{
    ...
    if (list_empty(&free_list->list) != 0) {
        goto out;
    }
...
STATIC struct davinci_intf_sub_module_stru *drv_davinci_get_module(const char *module_name)
{
    ...
    down_write(&g_davinci_intf_cb.cb_sem);
    if (list_empty(&g_davinci_intf_cb.module_list) != 0) {
        up_write(&g_davinci_intf_cb.cb_sem);
        return NULL;
    }
...
STATIC void drv_ascend_run_release_work(struct davinci_intf_free_list_stru *free_list)
{
    ...
    /* init the queue_work, start the work task */
    idx = (uint64_t)atomic64_inc_return(&g_recycle_wq_idx);
    INIT_WORK(&free_list->release_work, drv_davinci_run_release);
    queue_work(g_recycle_wq[idx % MAX_RECYCLE_WORKQUEUE], &free_list->release_work);
    return;
}
    ...
    struct file *file, unsigned int free_index)
{
    ...
    if (list_empty(&file_free_list->list) != 0) {
    ...
    list_for_each_entry_safe(file_pos, file_next, &file_free_list->list, list) {
        list_del(&file_pos->list);

        file_pos->free_index = free_index;
        file_pos->owner_list = proc_free_list;
        file_pos->owner_pid = proc->owner_pid;
        list_add_tail(&file_pos->list, &proc_free_list->list);
    }
...
int drv_ascend_add_release_list_all(struct davinci_intf_process_stru *proc, struct file *file)
{
    ...
    if (list_empty(&proc->file_list) != 0) {
        return 0;
    }
    ...
    list_for_each_entry_safe(node_info, node_info_next, module_list, list)
    {
        ...
        list_for_each_entry_safe(file_pos, file_next, file_list, list)
        {
            if ((strcmp(node_info->module_name, file_pos->module_name) != 0)) {
                if ((strcmp(DAVINIC_UNINIT_FILE, file_pos->module_name) == 0)) {
    ...
    unsigned long arg)
{
    ...
    if (copy_from_user(&module_para, (void *)(uintptr_t)arg,
    ...
    unsigned long arg)
{
    ...
    down_read(&g_davinci_intf_cb.cb_sem);
    module_para.status = check_module_file_close(&g_davinci_intf_cb, module_para.module_name);
    up_read(&g_davinci_intf_cb.cb_sem);
    if (copy_to_user((void *)(uintptr_t)arg, &module_para,
        sizeof(struct davinci_intf_check_no_use_arg)) != 0) {
    ...
    unsigned long addr, unsigned long len, unsigned long pgoff, unsigned long flags)
{
    if ((flags & MAP_FIXED) != 0) {
        log_intf_err("Not support MAP_FIXED flag. (flags=0x%lx)\n", flags);
        return -EINVAL;
    }
...
bool davinci_intf_confirm_user(void)
{
    ...
    for (i = 0; i < DAVINIC_CONFIRM_USER_NUM; i++) {
        ...
        if (in_group_p(*(kgid_t *)&g_authorized_group_id[i]) != 0) {
            valid_flag = true;
        }
    }
...
bool ascend_intf_is_restrict_access(struct file *filep)
{
    ...
    if (strcmp(filep->f_path.dentry->d_iname, DAVINCI_INTF_NPU_DEV_CUST_NAME) == 0) {
        return true;
    }

    return false;
}
EXPORT_SYMBOL(ascend_intf_is_restrict_access);

STATIC int drv_davinci_intf_recycle_wq_init(void)
{
    int i;
    atomic64_set(&g_recycle_wq_idx, 0);
    for (i = 0; i < MAX_RECYCLE_WORKQUEUE; i++) {
#ifdef CFG_FEATURE_BIND_CPU_ENABLE
        g_recycle_wq[i] = alloc_workqueue("intf_cb_recycle", WQ_UNBOUND, 1);
#else
        g_recycle_wq[i] = create_workqueue("intf_cb_recycle");
#endif
...
STATIC void drv_davinci_intf_recycle_wq_uninit(void)
{
    ...
    for (i = 0; i < MAX_RECYCLE_WORKQUEUE; i++) {
        if (g_recycle_wq[i] != NULL) {
            flush_workqueue(g_recycle_wq[i]);
            destroy_workqueue(g_recycle_wq[i]);
        }
        g_recycle_wq[i] = NULL;
    }
}
