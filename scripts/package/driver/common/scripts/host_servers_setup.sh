...
servers_autostart_centos() {
    ...
    if [ -f "$setupfile" ]; then
        ...
        cp -fp "$setupfile" $target_path/
        chkconfig --add host_sys_init.sh
...
case "$release_os" in
    ...
    Debian)
    if version_lt $release_version "10";then
        log "[WARNING]update-rc.d may not work on $release_os $release_version"
    fi
    servers_autostart_ubuntu
    ;;
    ...
    *)
    if [ "$release_os" = "openEuler" ]; then
        servers_autostart_centos
    elif [ -f "${Driver_Install_Path_Param}"/host_services_setup.sh ]; then
        "${Driver_Install_Path_Param}"/host_services_setup.sh "${release_os}"
        if [ $? -ne 0 ]; then
            log "[ERROR]host services setup failed."
            exit 1
        fi
    else
