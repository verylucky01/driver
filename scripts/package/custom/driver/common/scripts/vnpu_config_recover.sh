...
Ascend_server_board_id_list=("0x02" "0x11" "0x21" "0x27" "0x28" "0x24" "0x30" "0x31" "0x32" "0x33" "0x34" "0x50" "0x51" "0x52" "0x53")
...
{
    if [ -L "$VNPU_CONF" ]; then
        log "VNPU_CONF path is soft link, this has security risk, not take effect."
        return 1
    fi

    local user_id=$(stat -c %u ${VNPU_CONF})
    local group_id=$(stat -c %g ${VNPU_CONF})
...
function ismod_ipmi_driver()
{
    local has_find=0
    local card_id_cur
    local chip_id_cur
    local chip_type_is_910=0

    if [[ $(lspci | grep "d801") != "" ]]; then
        chip_type_is_910=1
    else
        log "The chip is not Ascend 910, do not need ismod ipmi driver."
        return 1
    fi

    sleep 1
    card_id_cur=`/usr/local/sbin/npu-smi info -t phyid-remap -p ${phy_id} | grep "NPU ID" |  awk -F ':' '{print $2}'`
    chip_id_cur=`/usr/local/sbin/npu-smi info -t phyid-remap -p ${phy_id} | grep "Chip ID" |  awk -F ':' '{print $2}'`
    local board_id_cur=$(/usr/local/sbin/npu-smi info -t board -i ${card_id_cur} -c ${chip_id_cur} | grep "Board ID" |  awk -F ': ' '{print $2}')

    for ((j = 0; j < ${#Ascend_server_board_id_list[*]}; j++))
    do
        if [[ "$board_id_cur" =~ "${Ascend_server_board_id_list[$j]}" ]]; then
            has_find=1
            break
        fi
    done

    if [ ${chip_type_is_910} -ne 1 ] || [ ${has_find} -ne 1 ]; then
        log "The board is not Ascend 910 server, do not need ismod ipmi driver."
        return 1
    fi
    
    local result=""
    result=$(lsmod | grep "ipmi_si")
    if [ x"$result" == x"" ]; then
        modprobe ipmi_si
        if [ $? -ne 0 ];then
            log "intall ipmi_si driver failed."
        fi
    fi

    result=$(lsmod | grep "ipmi_devintf")
    if [ x"$result" == x"" ]; then
        modprobe ipmi_devintf
        if [ $? -ne 0 ];then
            log "intall ipmi_devintf driver failed."
        fi
    fi

    result=$(lsmod | grep "ipmi_msghandler")
    if [ x"$result" == x"" ]; then
...
function config_recover()
{
    ...
    do
        if [ "$cfg_line" == "[vnpu-config start]" ]; then
            start=1
        fi

        if [ "$cfg_line" == "[vnpu-config end]" ]; then
            end=1
        fi
        
        if [ ${start} -eq 1 ] && [ ${end} -eq 0 ]; then
            cmd_info=`echo $cfg_line | grep -E "^${phy_id}:[0-9]{1,5}:npu-smi" | awk -F ':' '{print $3}'`
            if [ -n "$cmd_info" ];then
                vnpu_config=`echo ${cmd_info} | awk -F ' ' '{print $10}'`
                ...
                if [ "$vgroup_sign" == "-g" ];then
                    ...
                    cmd_info="npu-smi set -t create-vnpu -i ${card_id_cur} -c ${chip_id_cur} -f ${vnpu_config} -v ${vnpu_id}"
                fi
                log "now recover cfg by ${cmd_info} "
                ...
                do
                    ...
                    ret=$?
                    if [ $ret -eq 0 ];then
                        log "$j times cmd ${cmd_info} exec result is $ret"
                        break
                    fi
                    log "$j times cmd ${cmd_info} exec result is $ret"
                    sleep 10
