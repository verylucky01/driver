...
load_livepatch_conf() {
    ...
    if ! which livepatch >& /dev/null; then
        log "[INFO][LINENO=${LINENO}]The command [livepatch] not found."
...
output_local_drv_root_path() {
    ...
    cd "${g_livepatch_absolute_path}/../../../" >& /dev/null

    local_drv_root_path="${PWD}"
...
device_livepatch_operate_check() {
    ...
    if [ ! -e "${g_livepatch_absolute_path}/script/specific_func.inc" ]; then
        log "[INFO][LINENO=${LINENO}]The [${g_livepatch_absolute_path}/script/specific_func.inc] doesn't exist."
        return 0
    fi
