...
CUSTOM_OP_CONF="/etc/custom_op.cfg"
phy_id=$1
...
{
    if [ -L "$CUSTOM_OP_CONF" ]; then
        log "CUSTOM_OP_CONF path is soft link, this has security risk, not take effect."
        return 1
    fi

    local user_id=$(stat -c %u ${CUSTOM_OP_CONF})
    local group_id=$(stat -c %g ${CUSTOM_OP_CONF})
    
    if [[ $(lspci | grep "d803") != "" ]]; then
        log "The chip check is ok."
    else
        log "This device does not support custom-op command."
        return 1
    fi
...
function check_config_mode()
{
    while IFS= read -r cfg_line; do
        if [ "$cfg_line" == "custom-op-recover:enable" ]; then
            mode=1
            return 0
        fi
        if [ "$cfg_line" == "custom-op-recover:disable" ]; then
            mode=0
            return 0
        fi
    done < "${CUSTOM_OP_CONF}"
...
function config_recover()
{
    ...
    local customop_chip_id=0
    local customop_card_id=0
    local start=0
    local end=0
    ...
    while IFS= read -r cfg_line; do
        if [ "$cfg_line" == "[custom-op-config start]" ]; then
            start=1
            continue
        fi
        if [ "$cfg_line" == "[custom-op-config end]" ]; then
            end=1
            break
        fi
        if [ ${start} -eq 1 ] && [ ${end} -eq 0 ]; then
            if [ -n "$cfg_line" ]; then
                customop_card_id=`echo ${cfg_line} | awk -F ' ' '{print $6}'`
                customop_chip_id=`echo ${cfg_line} | awk -F ' ' '{print $8}'`
                enable_value=`echo ${cfg_line} | awk -F ' ' '{print $10}'`
                if [ -z "$customop_card_id" ] || [ -z "$customop_chip_id" ] || [ -z "$enable_value" ]; then
                    log "The configuration file was modified unexpectedly, please check it!"
                    continue
                fi
                if [[ ! "$customop_card_id" =~ ^[0-9]+$ ]] || [[ ! "$customop_chip_id" =~ ^[0-9]+$ ]] ||[[ ! "$enable_value" =~ ^[0-9]+$ ]]; then
                    log "Warning: error custom-op command, This configuration will not be restored."
                    continue
                fi
                if [ "${customop_card_id}" -lt 0 ] || [ "${customop_card_id}" -gt 7 ] || [[ "${customop_chip_id}" -ne 0 && "${customop_chip_id}" -ne 1 ]] || [[ "${enable_value}" -ne 0 && "${enable_value}" -ne 1 ]]; then
                    log "Warning: error custom-op command, This configuration will not be restored."
                    continue
                fi
                if [ "$customop_card_id" -ne "$card_id_cur" ] || [ "$customop_chip_id" -ne "$chip_id_cur" ] || [ "${enable_value}" -eq 0 ]; then
                    continue
                fi
                cmd_info="npu-smi set -t custom-op -i ${customop_card_id} -c ${customop_chip_id} -d ${enable_value}"
